...
import { parseModelArg, parsePositiveSafeInt, parseRecapCommand } from "../src/commands";
import { THINKING_LEVELS } from "../src/config";

assert.deepEqual(parseRecapCommand(""), { kind: "refresh" });
assert.deepEqual(parseRecapCommand("  \t\n"), { kind: "refresh" });
...
assert.notEqual(parseRecapCommand("garbage").kind, "refresh");
...
console.log("test-commands: passed");
