...
describe("recap configuration", () => {
  test("validates thinking levels and model references", () => {
    ...
    assert.equal(
      resolveRecapConfig({ recapModel: null, provider: "legacy", model: "legacy-model" })
        .recapModel,
      null
    );
    assert.equal(
    ...
    assert.deepEqual(
      resolveRecapConfig({ provider: " openrouter ", model: " model/id " }).recapModel,
  ...
  test("migrates legacy scheduling settings", () => {
    assert.equal(
  ...
  test("normalizes persisted settings and reads only global configuration", () => {
  ...
  test("persists settings atomically without replacing unrelated values", () => {
  ...
  test.each([
    ...
    ["array-root", "[1, 2, 3]\n"]
  ] as const)("refuses to overwrite %s settings", (name, original) => {
  ...
  test("cleans up a temporary file when atomic rename fails", () => {
    const agentDir = mkdtempSync(join(tmpdir(), "pi-recap-config-rename-failure-"));
    ...
    try {
      ...
      assert.throws(() => {
        saveRecapConfig(DEFAULT_CONFIG, agentDir);
      }, /rename failed/);
      assert.deepEqual(
        readdirSync(agentDir).filter((name) => name.endsWith(".tmp")),
        []
      );
    } finally {
